...
- **Research corrections folded in** (vs. earlier plan doc): MinIO OSS is archived/unmaintained (Apr 2026) — excluded; Backblaze B2 does not document conditional writes — S3 adapter needs HEAD-compare fallback; Nextcloud `If-Match` is undocumented — verify empirically, treat dufs as the reference WebDAV server; dufs needs ≥ 0.42.0 for `If-Match`.
