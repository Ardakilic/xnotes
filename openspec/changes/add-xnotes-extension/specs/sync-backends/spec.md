...
Sync settings SHALL validate input before activation: the endpoint MUST be a well-formed `http://` or `https://` URL; non-TLS endpoints (typical for LAN servers like dufs) SHALL be accepted but with a visible warning that credentials and data travel unencrypted; WebDAV requires username/password; S3 requires endpoint, region, bucket, non-empty key prefix, and access/secret keys. Activation SHALL be blocked while validation fails, with field-level messages.
...
#### Scenario: Plain-HTTP endpoint warned
- **WHEN** the user enters `http://192.168.1.10:5244` as a WebDAV endpoint
- **THEN** the settings are accepted but a visible unencrypted-connection warning is shown
