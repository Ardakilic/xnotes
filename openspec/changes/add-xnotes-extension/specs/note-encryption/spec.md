...
If the remote blob is an encryption envelope while encryption is disabled locally, the system SHALL fail with a typed error telling the user the remote data is encrypted and encryption must be enabled (with the passphrase) before syncing. If the remote blob is plaintext `StoreV2` while encryption is enabled locally, the system SHALL fail with a typed error warning that the remote data is not encrypted, and SHALL NOT push the encrypted local store over it automatically. The system SHALL provide a one-shot confirmation action that, after explicit user confirmation, permits exactly one conditional encrypted overwrite of the plaintext remote blob on the next sync cycle; the confirmation SHALL be required before uploading the encrypted envelope. If the confirmation is not given, the existing error and no-upload behavior SHALL be preserved. In all cases local data stays untouched and the user resolves the mismatch explicitly.
...
#### Scenario: Remote plaintext, local enabled, user confirms overwrite

- **WHEN** the user explicitly confirms the overwrite and the next sync cycle runs
- **THEN** the encrypted envelope is uploaded, replacing the plaintext remote blob, and the confirmation is consumed (not reusable for subsequent mismatches)
